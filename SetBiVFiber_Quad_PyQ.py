...
def SetBiVFiber_Quad_PyQ(param):
	...
	def lap(Me, boud, par,op, filename):
		...
		u_a = u.vector().array()
		u_a_quad = project(u, SQuad, solver_type = "mumps").vector().get_local()
		...
		L = inner(grad(u), v)*dx
		grad_u = Function(V_g)
		solve(a == L, grad_u, solver_parameters={"linear_solver": "mumps"})
		grad_u_quad = project(grad_u, VQuad, solver_type = "mumps")
